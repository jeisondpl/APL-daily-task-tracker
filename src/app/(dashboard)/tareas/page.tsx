...
import { MisPendientes } from '@/views/Tareas/MisPendientes'
import { PageHeader } from '@/shared/components/ui/PageHeader'
import { Button } from '@/shared/components/ui/Button'
import { formatFechaLarga, todayISO, parseLocalDate, toISODateLocal, monthRangeISO } from '@/shared/lib/utils'
import type { ICreateTareaDTO, ITarea, IUpdateTareaDTO } from '@/modules/tareas/domain/entities/Tarea.entities'
...
export default function TareasPage() {
  ...
  const [visibleMonth, setVisibleMonth] = useState<Date>(() => parseLocalDate(todayISO()))
  ...
  const dayTareas = useMemo(() => tareasCtrl.tareas.filter((t) => t.fecha === selectedDate), [tareasCtrl.tareas, selectedDate])
  ...
  return (
    <div>
      ...
      {listasCtrl.error && <p style={{ color: '#c0392b', fontSize: '0.875rem', marginBottom: '8px' }}>{listasCtrl.error}</p>}
        ...
        <div style={{ flex: 1, minWidth: '320px' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <button type='button' aria-label='Día siguiente' onClick={() => selectDate(shiftDay(selectedDate, 1))} className='dtt-cal-today-btn' style={dayNavBtn}>
                ›
              </button>
            </div>
          ...
          {tareasCtrl.error && <p style={{ color: '#c0392b', fontSize: '0.875rem', marginBottom: '8px' }}>{tareasCtrl.error}</p>}

          {tareasCtrl.loading ? <p style={{ color: 'var(--color-text-soft)' }}>Cargando tareas…</p> : <TareasGantt tareas={dayTareas} onSelectTarea={handleSelectTarea} />}
      ...
      <AgregarTareaModal open={createOpen} listas={listasCtrl.listas} fecha={selectedDate} onClose={() => setCreateOpen(false)} onCreate={handleCreate} />

      <EditarTareaModal tarea={selected} listas={listasCtrl.listas} open={editOpen} onClose={() => setEditOpen(false)} onSave={handleSave} onDelete={handleDelete} />
    </div>
  )
}
