...
export function ResumenDia({ tareas, listas }: { tareas: ITarea[]; listas: IListaTarea[] }) {
  const grupos = useGrupos(tareas, listas)
  const total = tareas.length

  // Excluir tareas de listas tipo pausa/almuerzo del cómputo de horas trabajadas
  const PAUSE_KEYWORDS = ['almuerzo', 'pausa', 'descanso', 'break', 'lunch']
  const pauseListIds = new Set(listas.filter((l) => PAUSE_KEYWORDS.some((kw) => l.nombre.toLowerCase().includes(kw))).map((l) => l.id))
  const tareasProductivas = tareas.filter((t) => !pauseListIds.has(t.listaId))
  const totalMin = tareasProductivas.reduce((acc, t) => acc + durMin(t), 0)
        ...
        <>
          ...
          >
            ...
            <div style={cardStyle}>
              <div style={kpiNumber}>
                {completadas}
                <span style={{ fontSize: '0.9rem', color: 'var(--color-text-soft)' }}>/{total}</span>
              </div>
          ...
          <div style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--color-text-soft)', marginBottom: 4 }}>Tareas por lista</div>
          <ResponsiveContainer width='100%' height={180}>
            <PieChart>
              <Pie data={grupos} dataKey='count' nameKey='nombre' innerRadius={42} outerRadius={72} paddingAngle={2} stroke='var(--color-surface)'>
                {grupos.map((g) => (
              ...
              <Tooltip
                formatter={(value, _name, item) => {
                  ...
                  return [`${value} tarea(s) · ${fmtHoras(g?.minutes ?? 0)}`, g?.nombre ?? '']
                }}
          ...
          <ul style={{ listStyle: 'none', margin: '8px 0 0', padding: 0, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {grupos.map((g) => (
              <li key={g.listaId} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.8rem' }}>
                ...
                <span style={{ color: 'var(--color-text)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{g.nombre}</span>
                <span style={{ color: 'var(--color-text-soft)', fontSize: '0.7rem' }}>{fmtHoras(g.minutes)}</span>
                <span style={{ color: 'var(--color-text-soft)', fontWeight: 600, minWidth: 16, textAlign: 'right' }}>{g.count}</span>
              </li>
            ))}
          </ul>
        </>
...
export function HorasPorListaBar({ tareas, listas }: { tareas: ITarea[]; listas: IListaTarea[] }) {
  const grupos = useGrupos(tareas, listas)
      ...
      {data.length === 0 ? (
        ...
        <ResponsiveContainer width='100%' height={Math.max(180, data.length * 46)}>
          <BarChart data={data} layout='vertical' margin={{ top: 4, right: 24, bottom: 4, left: 8 }}>
            ...
            <XAxis type='number' tick={{ fontSize: 12, fontFamily: FONT, fill: 'var(--color-text-soft)' }} tickFormatter={(v: number) => `${v}h`} />
            <YAxis type='category' dataKey='nombre' width={140} tick={{ fontSize: 12, fontFamily: FONT, fill: 'var(--color-text)' }} />
            <Tooltip
              cursor={{ fill: 'rgba(0,66,84,0.06)' }}
              formatter={(_value, _name, item) => {
                const p = item?.payload as { minutes: number; count: number } | undefined
                return [`${fmtHoras(p?.minutes ?? 0)} · ${p?.count ?? 0} tarea(s)`, 'Tiempo']
              }}
              contentStyle={{ fontFamily: FONT, fontSize: '0.8rem', borderRadius: 8 }}
            />
