...
export function ListasView() {
  const { listas, loading, error, _list, _create, _update, _delete } =
    useListasController();
  ...
  async function handleDelete(lista: IListaTarea) {
    if (
      !window.confirm(
        `¿Eliminar la lista "${lista.nombre}"? Se eliminarán también todas sus tareas.`
      )
    )
      return;
    ...
    <div>
            ...
            {listas.length === 0 ? (
              ...
              listas.map((lista) => (
                <Tr key={lista.id}>
                  ...
                  <Td style={{ color: "var(--color-text-soft)", fontSize: "0.75rem" }}>
                    {lista.colorDefault ?? "—"}
                  </Td>
                  <Td>
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {formError && (
            <p style={{ color: "var(--color-danger, #c0392b)", fontSize: "0.875rem" }}>
              {formError}
            </p>
          )}
          <Input
